...
import { useHumanReview } from "../runtime/assistantRuntime";
    ...
    <div className="mx-auto mb-4 w-full max-w-[44rem] rounded-xl border border-amber-200 bg-amber-50 p-4">
      <div className="mb-3">
        ...
        <p className="mt-1 text-xs leading-relaxed text-amber-800">
          Your request has been sent to a support representative for review.
        </p>
      </div>
      ...
      <div className="space-y-3 text-xs">
        {data.user_query && (
          <div>
            <span className="text-amber-700">Your request</span>
            <p className="mt-1 font-medium leading-relaxed text-amber-950">
              {data.user_query}
            </p>
          </div>
        )}
        ...
        <div className="grid grid-cols-2 gap-3">
          {data.intent && (
            <div>
              <span className="text-amber-700">Request type</span>
              <p className="mt-1 font-semibold capitalize text-amber-950">
  ...
  return (
    <ThreadPrimitive.ScrollToBottom
      render={<TooltipIconButton
        ...
        variant="outline"
        className="aui-thread-scroll-to-bottom dark:border-border dark:bg-background dark:hover:bg-accent absolute -top-12 z-10 self-center rounded-full p-4 disabled:invisible" />}><ArrowDownIcon /></ThreadPrimitive.ScrollToBottom>
  );
};
          ...
          <AuiIf condition={(s) => s.composer.dictation == null}>
            <ComposerPrimitive.Dictate
              render={<TooltipIconButton
                ...
                aria-label="Start voice input" />}><MicIcon className="aui-composer-dictate-icon size-4" /></ComposerPrimitive.Dictate>
          </AuiIf>
          <AuiIf condition={(s) => s.composer.dictation != null}>
            <ComposerPrimitive.StopDictation
              render={<TooltipIconButton
                ...
                aria-label="Stop voice input" />}><SquareIcon
        ...
        <AuiIf condition={(s) => !s.thread.isRunning}>
          <ComposerPrimitive.Send
            render={<TooltipIconButton
              ...
              aria-label="Send message" />}><ArrowUpIcon className="aui-composer-send-icon size-4" /></ComposerPrimitive.Send>
        </AuiIf>
        <AuiIf condition={(s) => s.thread.isRunning}>
          <ComposerPrimitive.Cancel
            render={<Button
              ...
              aria-label="Stop generating" />}><SquareIcon className="aui-composer-cancel-icon size-3.5 fill-current" /></ComposerPrimitive.Cancel>
        </AuiIf>
      </div>
    </div>
  );
    ...
    <ActionBarPrimitive.Root
      ...
      <ActionBarMorePrimitive.Root>
        <ActionBarMorePrimitive.Trigger
          render={<TooltipIconButton tooltip="More" className="data-[state=open]:bg-accent" />}><MoreHorizontalIcon /></ActionBarMorePrimitive.Trigger>
        <ActionBarMorePrimitive.Content
          ...
          <ActionBarPrimitive.ExportMarkdown
            render={<ActionBarMorePrimitive.Item
              className="aui-action-bar-more-item hover:bg-accent hover:text-accent-foreground focus:bg-accent focus:text-accent-foreground flex cursor-pointer items-center gap-2 rounded-lg px-2.5 py-1.5 text-sm outline-none select-none" />}><DownloadIcon className="size-4" />Export as Markdown
                              </ActionBarPrimitive.ExportMarkdown>
        </ActionBarMorePrimitive.Content>
      </ActionBarMorePrimitive.Root>
    </ActionBarPrimitive.Root>
  ...
  return (
    <ActionBarPrimitive.Root
      ...
      <ActionBarPrimitive.Edit
        render={<TooltipIconButton tooltip="Edit" className="aui-user-action-edit" />}><PencilIcon /></ActionBarPrimitive.Edit>
    </ActionBarPrimitive.Root>
  );
};
...
const EditComposer = () => {
  return (
    <MessagePrimitive.Root
      ...
      <ComposerPrimitive.Root
        className="aui-edit-composer-root border-border/60 dark:border-muted-foreground/15 ms-auto flex w-full max-w-[85%] cursor-text flex-col rounded-(--composer-radius) border bg-(--composer-bg)">
        <ComposerPrimitive.Input
        ...
        <div
          ...
          <ComposerPrimitive.Cancel
            render={<Button variant="ghost" size="sm" className="h-8 rounded-full px-3.5" />}>Cancel
                              </ComposerPrimitive.Cancel>
